...
import postsPageInitialPathsD1288Resource from '../../resources/posts-page-initial-paths-d1288'
import postsPageInitialProps5470aResource from '../../resources/posts-page-initial-props-5470a'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - Central Identity Architect</title>
          <meta
            property="og:title"
            content="Posts1 - Central Identity Architect"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPathsD1288Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialProps5470aResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
